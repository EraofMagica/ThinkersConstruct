...
public class ToolStatArgument<T extends IToolStat> implements ArgumentType<T> {
  private static final Collection<String> EXAMPLES = Arrays.asList("thconstruct:mining_speed", "thconstruct:durability");
  private static final DynamicCommandExceptionType NOT_FOUND = new DynamicCommandExceptionType(name -> dev.tocraft.thconstruct.ThConstruct.makeTranslation("command", "stat_type.not_found", name));
  private static final Dynamic2CommandExceptionType WRONG_TYPE = new Dynamic2CommandExceptionType((name, clazz) -> dev.tocraft.thconstruct.ThConstruct.makeTranslation("command", "stat_type.wrong_type", name, clazz));
