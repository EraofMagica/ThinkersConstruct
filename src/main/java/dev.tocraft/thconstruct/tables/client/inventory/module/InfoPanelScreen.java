package dev.tocraft.thconstruct.tables.client.inventory.module;
...
import dev.tocraft.eomantle.client.screen.SliderWidget;
import dev.tocraft.thconstruct.library.client.RenderUtils;
import dev.tocraft.thconstruct.tables.client.inventory.widget.BorderWidget;
...
public class InfoPanelScreen extends ModuleScreen {
  ...
  private static final Component DEFAULT_CAPTION = dev.tocraft.thconstruct.ThConstruct.makeTranslation("gui", "caption").withStyle(ChatFormatting.UNDERLINE);

  protected static ResourceLocation BACKGROUND_IMAGE = dev.tocraft.thconstruct.ThConstruct.getResource("textures/gui/panel.png");
  ...
  protected void renderTooltip(GuiGraphics matrices, int mouseX, int mouseY) {
    ...
    if (this.hasTooltips() && mouseX >= this.guiRight() - this.border.w - this.font.width("?") / 2 && mouseX < this.guiRight()
      ...
      this.renderTooltip(matrices, this.font.split(Component.translatable("gui.thconstruct.general.hover"), 150), mouseX - 155, mouseY);
    }
