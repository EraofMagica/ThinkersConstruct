package dev.tocraft.thconstruct.library.tools.capability;

import dev.tocraft.eomantle.data.registry.IdAwareComponentRegistry;
import dev.tocraft.thconstruct.library.modifiers.data.FloatMultiplier;
...
public interface TinkerDataKeys {
  ...
  TinkerDataKey<Integer> SHOW_EMPTY_OFFHAND = dev.tocraft.thconstruct.ThConstruct.createKey("show_empty_offhand"); // unregistered as ShowOffhandModule exists

	/** Float value for the FOV modifier, will be 1.0 if no change */
	ComputableDataKey<FloatMultiplier> FOV_MODIFIER = dev.tocraft.thconstruct.ThConstruct.createKey("zoom_multiplier", FloatMultiplier::new);

	/** FOV modifier that only applies when not disabled in the settings menu */
	ComputableDataKey<FloatMultiplier> SCALED_FOV_MODIFIER = dev.tocraft.thconstruct.ThConstruct.createKey("scaled_fov_multiplier", FloatMultiplier::new);
	...
	/** Bonus to add to movement speed when using items, used in {@link dev.tocraft.thconstruct.tools.TinkerModifiers#projectileProtection} */
  ...
  private static TinkerDataKey<Integer> intKey(String name) {
    return INTEGER_REGISTRY.register(dev.tocraft.thconstruct.ThConstruct.createKey(name));
  }

  /** Creates and registers a float key */
  private static TinkerDataKey<Float> floatKey(String name) {
    return FLOAT_REGISTRY.register(dev.tocraft.thconstruct.ThConstruct.createKey(name));
  }
}
