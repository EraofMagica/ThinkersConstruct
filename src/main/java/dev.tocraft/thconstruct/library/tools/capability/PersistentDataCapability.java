...
import dev.tocraft.thconstruct.common.network.SyncPersistentDataPacket;
...
public class PersistentDataCapability {
  ...
  /** Capability ID */
  private static final ResourceLocation ID = dev.tocraft.thconstruct.ThConstruct.getResource("persistent_data");
  /** Capability type */
  ...
  public static NamespacedNBT getOrWarn(Entity entity) {
    ...
    if (data.isEmpty()) {
      dev.tocraft.thconstruct.ThConstruct.LOG.warn("Missing Tinkers NBT on entity {}, this should not happen", entity.getType());
