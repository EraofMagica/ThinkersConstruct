package dev.tocraft.thconstruct.library.tools.nbt;
...
import dev.tocraft.thconstruct.library.modifiers.hook.build.ModifierTraitHook.TraitBuilder;
import dev.tocraft.thconstruct.library.tools.SlotType;
import dev.tocraft.thconstruct.library.tools.context.ToolRebuildContext;
...
public class ToolStack implements IToolStackView {
  ...
  private static final String KEY_VALIDATE_SLOTS = dev.tocraft.thconstruct.ThConstruct.makeTranslationKey("recipe", "modifier.validate_slots");
      ...
      if (!copyNbt) {
        ...
        if (definition != ToolDefinition.EMPTY) {
          ...
          switch (Config.COMMON.logInvalidToolStack.get()) {
            case STACKTRACE ->
              dev.tocraft.thconstruct.ThConstruct.LOG.warn("Tool stack constructed using non-modifiable tool, this may cause issues as it has no NBT. Stacktrace can be disabled in config.", new Exception("Stack trace"));
            case WARNING ->
              dev.tocraft.thconstruct.ThConstruct.LOG.warn("Tool stack constructed using non-modifiable tool, this may cause issues as it has no NBT. To debug this issue or disable the warning, use logInvalidToolStack in the config.");
          }
        }
      }
    } else if (copyNbt) {
