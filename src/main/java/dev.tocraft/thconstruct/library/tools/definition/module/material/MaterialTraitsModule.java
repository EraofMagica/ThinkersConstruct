...
public final class MaterialTraitsModule implements ToolTraitHook, ToolModule {
  ...
  private static final MaterialStatsId MISSING = new MaterialStatsId(dev.tocraft.thconstruct.ThConstruct.MOD_ID, "missingno");
  ...
  private MaterialStatsId getStatType(ToolDefinition definition) {
    if (statType == null) {
      ...
      if (materialIndex < statTypes.size()) {
        ...
        dev.tocraft.thconstruct.ThConstruct.LOG.error("Invalid material index {} for {} traits module, total materials {}", materialIndex, definition.getId(), statTypes.size());
        statType = MISSING;
      }
    }
    return statType;
