...
import dev.tocraft.eomantle.data.loadable.IAmLoadable;
import dev.tocraft.eomantle.data.loadable.field.LoadableField;
import dev.tocraft.eomantle.data.loadable.mapping.EitherLoadable;
...
public class MaterialRepairModule implements MaterialRepairToolHook, ToolModule, IAmLoadable.Record {
  ...
  public static int getDurability(@Nullable ResourceLocation toolId, MaterialId material, MaterialStatsId statType) {
    ...
    if (stats instanceof IRepairableMaterialStats repairable) {
      ...
      if (toolId != null) {
        dev.tocraft.thconstruct.ThConstruct.LOG.warn("Attempting to repair {} using {}, but stat type {}{}. This usually indicates a broken datapack.", toolId, material, statType, stats == null ? " does not exist for the material" : " does not contain durability");
      }
      return 0;
    }
  }
