package dev.tocraft.thconstruct.library.client.armor;
  ...
  protected void apply(Map<ResourceLocation,JsonElement> splashList, ResourceManager manager, ProfilerFiller pProfiler) {
    ...
    for (Entry<ResourceLocation,JsonElement> entry : splashList.entrySet()) {
      ...
      try {
        ...
        dev.tocraft.thconstruct.ThConstruct.LOG.error("Failed to load armor model from {}", key, e);
      }
    }
    ...
    if (!missing.isEmpty()) {
      dev.tocraft.thconstruct.ThConstruct.LOG.error("Missing armor models used by items: {}", missing);
    }
    dev.tocraft.thconstruct.ThConstruct.LOG.info("Loaded {} armor models in {} ms", models.size(), (System.nanoTime() - time) / 1000000f);
  }
    ...
    protected ArmorModel getModel(ItemStack stack) {
      if (model == null) {
        ...
        if (model == ArmorModel.EMPTY) {
          dev.tocraft.thconstruct.ThConstruct.LOG.warn("Failed to find armor model {}, will skip rendering {}", getName(), stack);
        }
      }
      return model;
    }
