...
import static dev.tocraft.thconstruct.library.client.model.tools.ToolModel.registerSmallTool;
...
public class TinkerTransformTypes {
  ...
  private static TransformType create(String name, TransformType fallback) {
    ...
    if (fallback == TransformType.NONE) {
      return TransformType.create(key, dev.tocraft.thconstruct.ThConstruct.getResource(name));
    }
    return TransformType.create(key, dev.tocraft.thconstruct.ThConstruct.getResource(name), fallback);
  }
}
