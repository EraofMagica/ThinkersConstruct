package dev.tocraft.thconstruct.library.client.model.tools;
...
import dev.tocraft.eomantle.client.model.util.MantleItemLayerModel;
import dev.tocraft.eomantle.data.loadable.Loadable;
import dev.tocraft.eomantle.data.loadable.mapping.CompactLoadable;
...
import dev.tocraft.eomantle.util.ReversedListBuilder;
import dev.tocraft.thconstruct.library.client.materials.MaterialRenderInfo.TintedSprite;
import dev.tocraft.thconstruct.library.client.materials.MaterialRenderInfoLoader;
import dev.tocraft.thconstruct.library.client.model.BakedUniqueGuiModel;
import dev.tocraft.thconstruct.library.client.modifiers.IBakedModifierModel;
...
public class ToolModel implements IUnbakedGeometry<ToolModel> {
  ...
  private static void addModifierQuads(Function<Material, TextureAtlasSprite> spriteGetter, Map<ModifierId,IBakedModifierModel> modifierModels, List<FirstModifier> firstModifiers, IToolStackView tool, Consumer<Collection<BakedQuad>> quadConsumer, @Nullable ItemLayerPixels pixels, Transformation transforms, boolean isLarge) {
    if (!modifierModels.isEmpty()) {
      ...
      List<ModifierEntry> modifiers = tool.getUpgrades().modifiers;
      ...
      if (!modifiers.isEmpty()) {
        ...
        Set<ModifierId> hidden = ModifierSetWorktableRecipe.getModifierSet(tool.getPersistentData(), dev.tocraft.thconstruct.ThConstruct.getResource("invisible_modifiers"));
        for (int i = modifiers.size() - 1; i >= 0; i--) {
          ModifierEntry entry = modifiers.get(i);
  ...
  private static BakedModel bakeInternal(IGeometryBakingContext owner, Function<Material, TextureAtlasSprite> spriteGetter, @Nullable Transformation largeTransforms,
    ...
    if (particle == null) {
      ...
      dev.tocraft.thconstruct.ThConstruct.LOG.error("Created tool model without a particle sprite, this means it somehow has no parts. This should not be possible");
    }
  ...
  public static final class MaterialOverrideHandler extends ItemOverrides {
    ...
    public BakedModel resolve(BakedModel originalModel, ItemStack stack, @Nullable ClientLevel world, @Nullable LivingEntity entity, int seed) {
      ...
      try {
        ...
        dev.tocraft.thconstruct.ThConstruct.LOG.error("Failed to get tool model from cache", e);
        return originalModel;
      }
    }
  }
