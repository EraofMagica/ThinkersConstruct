...
public class HarvestTiers {
  ...
  private static MutableComponent makeLevelKey(Tier tier) {
    ...
    return dev.tocraft.thconstruct.ThConstruct.makeTranslation("stat", key).withStyle(style -> style.withColor(color));
  }
  ...
  public static Tier minTier() {
    ...
    if (sortedTiers.isEmpty()) {
      dev.tocraft.thconstruct.ThConstruct.LOG.error("No sorted tiers exist, this should not happen");
      return Tiers.WOOD;
    }
