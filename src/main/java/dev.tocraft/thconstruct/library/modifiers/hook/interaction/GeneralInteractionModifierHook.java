package dev.tocraft.thconstruct.library.modifiers.hook.interaction;
...
public interface GeneralInteractionModifierHook {
  ...
  ResourceLocation KEY_ACTIVE_MODIFIER = dev.tocraft.thconstruct.ThConstruct.getResource("active_modifier");
  /** Persistent data key storing the drawtime needed for using the tool. Generally is set when tool usage starts */
  ResourceLocation KEY_DRAWTIME = dev.tocraft.thconstruct.ThConstruct.getResource("drawtime");
  ...
  /**
   * Use in {@link net.minecraft.world.item.Item#use(Level, Player, InteractionHand)} or {@link #onToolUse(IToolStackView, ModifierEntry, Player, InteractionHand, InteractionSource)} to setup draw time for {@link dev.tocraft.thconstruct.library.client.model.TinkerItemProperties}.
   * @param tool      Tool being used
   * @param living    Entity using the tool, used for the vanilla hook
