package dev.tocraft.thconstruct.library.recipe.alloying;
  ...
  public void performRecipe(IMutableAlloyTank inv) {
    ...
    if (inv.canFit(output, drainAmount)) {
      ...
      for (int i = 0; i < drainFluids.length; i++) {
        ...
        if (toDrain != null) {
          ...
          if (drained.getAmount() != toDrain.getAmount()) {
            dev.tocraft.thconstruct.ThConstruct.LOG.error("Wrong amount of fluid {} drained for recipe {}", drained.getFluid(), id);
          }
        }
      }
      ...
      if (filled != output.getAmount()) {
        dev.tocraft.thconstruct.ThConstruct.LOG.error("Filled only {} for recipe {}", filled, id);
      }
    }
  }
