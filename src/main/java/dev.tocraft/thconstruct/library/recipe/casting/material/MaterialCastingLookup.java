...
import dev.tocraft.thconstruct.common.recipe.RecipeCacheInvalidator.DuelSidedListener;
...
public class MaterialCastingLookup {
  ...
  public static void registerItemCost(IMaterialItem item, int cost) {
    ...
    if (ITEM_COST_LOOKUP.containsKey(item)) {
      ...
      if (cost != original) {
        dev.tocraft.thconstruct.ThConstruct.LOG.error("Inconsistent cost for item {}", Registry.ITEM.getKey(item.asItem()));
