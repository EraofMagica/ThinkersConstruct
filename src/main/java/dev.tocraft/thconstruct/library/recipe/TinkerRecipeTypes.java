...
public class TinkerRecipeTypes {
  /** Deferred instance */
  private static final DeferredRegister<RecipeType<?>> TYPES = DeferredRegister.create(Registry.RECIPE_TYPE_REGISTRY, dev.tocraft.thconstruct.ThConstruct.MOD_ID);
  ...
  static <T extends Recipe<?>> RegistryObject<RecipeType<T>> register(String name) {
    return TYPES.register(name, () -> new RecipeType<>() {
      ...
      public String toString() {
        return dev.tocraft.thconstruct.ThConstruct.MOD_ID + ":" + name;
      }
    });
  }
}
