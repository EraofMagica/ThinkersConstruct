package dev.tocraft.thconstruct.common.data.model;
...
import static dev.tocraft.eomantle.util.IdExtender.INSTANCE;
...
public class TinkerBlockStateProvider extends BlockStateProvider {
  ...
  protected ResourceLocation blockTexture(String path) {
    return new ResourceLocation(dev.tocraft.thconstruct.ThConstruct.MOD_ID, ModelProvider.BLOCK_FOLDER + "/" + path);
  }

  /** Creates a texture in the block folder */
  protected ResourceLocation itemTexture(String path) {
    return new ResourceLocation(dev.tocraft.thconstruct.ThConstruct.MOD_ID, ModelProvider.ITEM_FOLDER + "/" + path);
  }
  ...
  public void axisBlock(Block block, String location, ResourceLocation texture, boolean horizontal) {
    ...
    ModelFile model = models().cubeColumn(dev.tocraft.thconstruct.ThConstruct.resourceString(location), texture, endTexture);
    axisBlock((RotatedPillarBlock)block, model,
              horizontal ? models().cubeColumnHorizontal(dev.tocraft.thconstruct.ThConstruct.resourceString(location + "_horizontal"), texture, endTexture) : model);
    simpleBlockItem(block, model);
  }
