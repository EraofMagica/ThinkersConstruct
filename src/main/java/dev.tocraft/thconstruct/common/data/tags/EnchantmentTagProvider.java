...
public class EnchantmentTagProvider extends TagsProvider<Enchantment> {
  public EnchantmentTagProvider(DataGenerator generator, @Nullable ExistingFileHelper existingFileHelper) {
    super(generator, Registry.ENCHANTMENT, dev.tocraft.thconstruct.ThConstruct.MOD_ID, existingFileHelper);
  }
  ...
  private void modifierTag(ModifierId modifier, String... ids) {
    TagsProvider.TagAppender<Enchantment> appender = tag(TagKey.create(Registry.ENCHANTMENT_REGISTRY, dev.tocraft.thconstruct.ThConstruct.getResource("modifier_like/" + modifier.getPath())));
    for (String id : ids) {
