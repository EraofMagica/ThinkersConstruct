...
public class SlimeColorReloadListener extends SimplePreparableReloadListener<int[]> {
  ...
  public SlimeColorReloadListener(FoliageType color) {
    this.color = color;
    this.path = dev.tocraft.thconstruct.ThConstruct.getResource("textures/colormap/" + color.getSerializedName() + "_grass_color.png");
  }
  ...
  protected int[] prepare(ResourceManager resourceManager, ProfilerFiller profiler) {
    ...
    try {
      ...
      dev.tocraft.thconstruct.ThConstruct.LOG.error("Failed to load slime colors", ioexception);
      return new int[0];
    }
  }
