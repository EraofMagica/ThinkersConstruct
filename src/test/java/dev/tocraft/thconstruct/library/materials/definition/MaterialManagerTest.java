package dev.tocraft.thconstruct.library.materials.definition;
...
class MaterialManagerTest extends BaseMcTest {
  ...
  void loadFullMaterial_allStatsPresent() {
    ...
    assertThat(testMaterial.getIdentifier()).isEqualByComparingTo(new MaterialId("thconstruct", "full"));
  ...
  void loadMinimalMaterial_succeedWithDefaults() {
    ...
    assertThat(allMaterials).hasSize(1);
    IMaterial testMaterial = allMaterials.iterator().next();
    assertThat(testMaterial.getIdentifier()).isEqualByComparingTo(new MaterialId("thconstruct", "minimal"));
  ...
  @Test
  void failOnMissing() {
    ResourceLocation materialId = dev.tocraft.thconstruct.ThConstruct.getResource("nonexistant");
  ...
  void redirect_toExisting() {
    ...
    Collection<IMaterial> allMaterials = materialManager.getAllMaterials();
    assertThat(allMaterials).hasSize(1);
    assertThat(materialManager.resolveRedirect(new MaterialId("thconstruct:redirect_always"))).isEqualTo(new MaterialId("thconstruct:full"));
  }
  ...
  void redirect_toNonexisting() {
    ...
    Collection<IMaterial> allMaterials = materialManager.getAllMaterials();
    assertThat(allMaterials).isEmpty();
    assertThat(materialManager.resolveRedirect(new MaterialId("thconstruct:redirect_always"))).isEqualTo(new MaterialId("thconstruct:redirect_always"));
  }
  ...
  void redirect_withMultipleConditions() {
    ...
    assertThat(materialManager.resolveRedirect(new MaterialId("thconstruct:redirect_conditional"))).isEqualTo(new MaterialId("thconstruct:minimal"));
  }
}
